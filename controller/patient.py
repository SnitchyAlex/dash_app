...
def _register_form_callbacks(app):
    ...
    def save_glicemia_measurement(n_clicks, valore, data_misurazione, ora, momento_pasto, note, due_ore_pasto):
        ...
        try:
            ...
            #if exists(g for g in Glicemia if g.paziente == paziente and g.data_ora == data_ora):
             #   return get_error_message(
              #      "Hai già una misurazione registrata a questa data e ora. "
               #      "Modifica l’orario (anche di 1 minuto) oppure aggiorna quella esistente."
                #     ), dash.no_update
...
def _register_data_callbacks(app):
    ...
    def load_patient_therapies(children):
        ...
        try:
            ...
            terapie = list(paziente.terapies) if hasattr(paziente, 'terapies') else list(getattr(paziente, 'terapie', []))
...
def _register_chart_callbacks(app):
    ...
    def render_week_month_charts(_children, weeks_window):
        ...
        # Recupera misurazioni (tollerante a nomi di relazione diversi)
        all_meas = _rel_list(paziente, 'glicemias', 'glicemie', 'rilevazione', 'rilevazioni')
        if not all_meas:
...
def _create_empty_figures(message="Nessuna glicemia registrata"):
    """Crea tre figure vuote con messaggio (istanze distinte)"""
    def empty_fig():
        f = go.Figure()
...
def _rel_list(paziente, *names):
    """
    Restituisce list(relazione) provando più possibili nomi senza far esplodere Pony.
    Esempio: _rel_list(paziente, 'glicemias', 'glicemie', 'rilevazione', 'rilevazioni')
    """
    for n in names:
        if hasattr(paziente, n):
            try:
                return list(getattr(paziente, n))
            except Exception:
                pass
    return []
...
def _check_patient_alerts(paziente):
    """
    Verifica gli alert per il paziente usando Python puro (evita errori interni di Pony).
    """
    alerts = []
    ...
    try:
        assunzioni_oggi = [
            a for a in _rel_list(paziente, 'assunzioni', 'assunzione', 'assunzione_set')
            if start <= a.data_ora < end
        ]
    ...
    try:
        terapie = _rel_list(paziente, 'terapie', 'terapies')
    except Exception:
        terapie = []
    ...
    try:
        glicemie_oggi = [
            g for g in _rel_list(paziente, 'glicemias', 'glicemie', 'rilevazione', 'rilevazioni')
...
def _register_alert_callbacks(app):
    ...
    # (2) Clic campanellina -> apri/chiudi Modal e popola contenuto (aggiornati con alerts-refresh)
    @app.callback(
        ...
        Input('alerts-refresh', 'data'),        # <— NEW: aggiorna colore subito dopo i salvataggi
        State('alerts-modal', 'is_open'),
        prevent_initial_call=False
    )
    @db_session
    def toggle_alerts_modal(n_bell, n_close, content_update, _refresh, is_open):
        ...
        if trigger_id == 'bell-button':
            ...
            return False, body_children, bell_color
        elif trigger_id in ('patient-content', 'alerts-refresh'):
            # Solo aggiorna contenuto e colore, non aprire/chiudere
